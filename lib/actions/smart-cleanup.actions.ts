...
export const analyzeReports = withAuth(async (user) => {
  try {
    ...
    console.log(
      `✅ Analysis complete: ${analysisResult.analyzedFiles} files analyzed`,
    );
...
export const executeSingleRecommendation = withAuth(
  async (user, recommendation: any) => {
    try {
      const companyId = user.user_metadata.companyId;
      if (!companyId) {
        throw new Error("Company ID not found");
      ...
      console.log(
        `🎯 Executing single recommendation: ${recommendation.action} for ${recommendation.filePath}`,
      );
        ...
        case "ARCHIVE":
          ...
          await prisma.reportCleanupLog.create({
            data: {
              ...
              filePath: recommendation.filePath,
              action: "ARCHIVE",
              spaceSaved: Math.floor(
                (recommendation.potentialSavings || 0) * 0.3,
              ), // Assume 30% savings from archiving
              reasoning: recommendation.reasoning,
              confidence: recommendation.confidence,
              executedBy: user.id,
            },
        ...
        case "COMPRESS":
          ...
          await prisma.reportCleanupLog.create({
            data: {
              ...
              action: "COMPRESS",
              spaceSaved: Math.floor(
                (recommendation.potentialSavings || 0) * 0.5,
              ), // Assume 50% savings from compression
              reasoning: recommendation.reasoning,
              confidence: recommendation.confidence,
              executedBy: user.id,
            },
      ...
      return {
        ...
        data: result,
        message:
          result.message || `${recommendation.action} executed successfully`,
      };
    } catch (error) {
...
export const executeAllRecommendations = withAuth(
  async (user, recommendations: any[]) => {
    try {
      const companyId = user.user_metadata.companyId;
      if (!companyId) {
        throw new Error("Company ID not found");
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Failed to fetch protection rules",
    };
  }
});

// Remove protection from a file
export const removeProtection = withAuth(
  async (user, protectionRuleId: string) => {
    try {
      const companyId = user.user_metadata.companyId;
      if (!companyId) {
        throw new Error("Company ID not found");
