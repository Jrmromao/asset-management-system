...
export const create = async (values: z.infer<typeof accessorySchema>): Promise<ApiResponse<Accessory>> => {
    try {
...
const computeQuantities = (accessory: Accessory) => {
    const totalStock = accessory.totalQuantityCount;
    const assignedQuantity = accessory?.userAccessories?.reduce((sum: number, assignment: UserAccessory) => sum + assignment.quantity, 0);
    return {
        totalStock,
        assignedQuantity,
...
export async function assign(values: z.infer<typeof assignmentSchema>): Promise<ApiResponse<Accessory>> {
    try {
        try {
        ...
        return await prisma.$transaction(async (tx) => {
            ...
            if (availableQuantity < requestedQuantity) {
                throw new Error(`Not enough quantity available. Requested: ${requestedQuantity}, Available: ${availableQuantity}`);
            }

            // Create user accessory assignment
            const assignment = await tx.userAccessory.create({
                data: {
...
export async function unassign(values: z.infer<typeof unassignSchema>): Promise<ApiResponse<Accessory>> {
    try {
        ...
        return await prisma.$transaction(async (tx) => {
            ...
            if (!currentAssignment) {
                throw new Error('No active assignment found for this user and accessory');
            }
        ...
        return {
            error: error instanceof Error ? error.message : 'Failed to unassign Accessory'
        };
    }
}
