...
} from "@/lib/services/ai-cost-optimization.service";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log("🔐 Cost Optimization API: Auth check completed", {
      userId: userId ? "✅" : "❌",
      orgId,
    });
    ...
    console.log("📊 Cost Optimization API: Request body parsed", {
      analysisType,
      timeframe,
      body,
    });
    ...
    if (!analysisType || !["license", "accessory"].includes(analysisType)) {
      console.log("❌ Cost Optimization API: Invalid analysis type", {
        analysisType,
      });
    ...
    if (!user) {
      console.log(
        "🔍 Cost Optimization API: User not found by ID, trying to find by email",
      );
      ...
      if (clerkUser.ok) {
        ...
        if (email) {
          ...
          console.log("📧 Cost Optimization API: User lookup by email", {
            found: !!user,
            email,
          });
        }
    ...
    if (!user) {
      console.log(
        "🧪 Cost Optimization API: No user found, using test user for development",
      );
    ...
    console.log("🚀 Cost Optimization API: Starting analysis", {
      type: analysisType,
    });
    let result;
