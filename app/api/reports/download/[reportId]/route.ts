...
export async function GET(
  ...
  try {
    ...
    if (generatedReport.status === "failed") {
      return NextResponse.json(
        { error: "Report generation failed. Please try generating the report again." },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      const files = await s3Service.listFiles(generatedReport.configuration.companyId, s3Key);
      ...
      const fileKey = reportFile.Key.replace(`companies/${generatedReport.configuration.companyId}/`, '');
      
      // Download file from S3
      const s3Object = await s3Service.downloadFile(generatedReport.configuration.companyId, fileKey);
