...
export function useAccessoryForm() {
    const { data: manufacturers, isLoading: isLoadingManufacturers } = useFormData({
        fetchFn: getAllManufacturers,
        onError: (error) => console.error('Failed to fetch manufacturers:', error)
    });
    ...
    const isLoading = isLoadingManufacturers || isLoadingSuppliers || isLoadingCategories;
