...
export const useItemDetails = ({
  itemId,
  itemType,
}: UseItemDetailsProps): UseItemDetailsReturn => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const relationshipsPromise = fetch(
        `/api/${itemType}s/${itemId}/relationships`,
      );
      const attachmentsPromise = fetch(
        `/api/${itemType}s/${itemId}/attachments`,
      );
      ...
      setError(
        err instanceof Error ? err : new Error("Failed to fetch item details"),
      );
    } finally {
      setIsLoading(false);
    }
