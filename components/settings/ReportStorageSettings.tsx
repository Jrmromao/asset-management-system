...
import { getProtectionRules, removeProtection } from "@/lib/actions/smart-cleanup.actions";
...
const ReportStorageSettings = () => {
  ...
  const [isProtectedFilesCollapsed, setIsProtectedFilesCollapsed] = useState(true);
  const [isTraditionalCleanupCollapsed, setIsTraditionalCleanupCollapsed] = useState(true);
  ...
  const executeRecommendation = async (recommendation: any) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Mark as completed
        setCompletedRecommendations((prev: Set<string>) => new Set([...Array.from(prev), recommendation.filePath]));
        ...
        if (recommendation.action === 'PROTECT') {
          ...
          toast({
                ...
                <div>
                  <div className="font-semibold text-gray-900 flex items-center gap-2">
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="w-1 h-1 bg-emerald-500 rounded-full animate-pulse ml-1" style={{animationDelay: '0.2s'}}></div>
                      <div className="w-1 h-1 bg-emerald-500 rounded-full animate-pulse ml-1" style={{animationDelay: '0.4s'}}></div>
                    </div>
                  </div>
              ...
              <div className="mt-4 space-y-3">
                <div className="grid grid-cols-1 gap-2">
                  <div className="flex items-center gap-3 text-sm text-gray-700 bg-emerald-50 px-3 py-2 rounded-lg border border-emerald-100">
                    ...
                    <span className="font-medium">Permanently protected from cleanup</span>
                  </div>
                  <div className="flex items-center gap-3 text-sm text-gray-700 bg-blue-50 px-3 py-2 rounded-lg border border-blue-100">
                    ...
                    <span className="font-medium">Added to protection rules database</span>
                  </div>
                  <div className="flex items-center gap-3 text-sm text-gray-700 bg-purple-50 px-3 py-2 rounded-lg border border-purple-100">
                    ...
                    <span className="font-medium">Smart cleanup engine updated</span>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-200 rounded-xl p-4 mt-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="text-sm text-emerald-800">
                      <div className="font-semibold mb-1">🎯 Protection Active</div>
                      <div className="text-emerald-700 leading-relaxed">
                        This file is now <strong>permanently protected</strong> and will be excluded from all future cleanup operations. 
                        It will remain safely stored regardless of age, size, or usage patterns.
                      </div>
            ...
            duration: 8000,
            className: "border-emerald-200 bg-white shadow-2xl shadow-emerald-500/10 max-w-md",
          });
        } else {
          ...
          toast({
            ...
            description: actionMessages[recommendation.action as keyof typeof actionMessages] || `${recommendation.action} completed for ${recommendation.filePath.split('/').pop()}`,
          });
        }
        ...
        if (recommendation.action === 'PROTECT') {
          ...
          setSmartCleanupAnalysis((prev: any) => {
            ...
            return {
              ...prev,
              recommendations: prev.recommendations.filter((r: any) => r.filePath !== recommendation.filePath)
            };
          });
        }
            ...
            <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
              <div className="flex items-center justify-between mb-3">
                <h4 className="font-medium text-gray-900 flex items-center gap-2">
                  ...
                  AI Recommendations ({smartCleanupAnalysis.recommendations.length})
                </h4>
                <Button
                  size="sm"
                  onClick={executeAllRecommendations}
                  disabled={isSmartCleanupRunning || smartCleanupAnalysis.recommendations.length === 0}
              ...
              <div className="space-y-2">
                {(showAllRecommendations 
                  ...
                  const isCompleted = completedRecommendations.has(rec.filePath);
                        ...
                        isCompleted 
                          ...
                          : isExecuting 
                            ...
                            : rec.action === 'DELETE' ? 'bg-red-500' :
                              rec.action === 'ARCHIVE' ? 'bg-amber-500' :
                              rec.action === 'COMPRESS' ? 'bg-blue-500' : 'bg-emerald-500'
                      }`}></div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2">
                          <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium ${
                            rec.action === 'DELETE' ? 'bg-red-100 text-red-800' :
                            rec.action === 'ARCHIVE' ? 'bg-amber-100 text-amber-800' :
                            rec.action === 'COMPRESS' ? 'bg-blue-100 text-blue-800' : 'bg-emerald-100 text-emerald-800'
                          }`}>
                            {rec.action}
                          </span>
                          <span className="text-sm text-gray-600 truncate">{rec.filePath.split('/').pop()}</span>
                        </div>
                        <p className="text-sm text-gray-700 mb-2 leading-relaxed">{rec.reasoning}</p>
                    ...
                    <div className="flex flex-col items-end gap-3 ml-4">
                      <div className="flex flex-col items-end gap-2">
                        <Badge variant="outline" className={`text-xs font-medium ${
                          rec.confidence > 0.8 ? 'border-emerald-300 text-emerald-700 bg-emerald-50' :
                          rec.confidence > 0.6 ? 'border-amber-300 text-amber-700 bg-amber-50' :
                      ...
                      <Button
                        ...
                        variant={
                          isCompleted ? 'outline' :
                          rec.action === 'DELETE' ? 'destructive' : 
                          rec.action === 'PROTECT' ? 'secondary' : 'default'
                        }
                        ...
                        disabled={isSmartCleanupRunning || isExecuting || isCompleted}
                      >
                        {isCompleted ? (
                          <>
                            <CheckCircle2 className="w-3 h-3 mr-1" />
                            {rec.action === 'PROTECT' ? 'Protected' : 'Completed'}
                          </>
                        ) : isExecuting ? (
                          <>
                            <RefreshCw className="w-3 h-3 mr-1 animate-spin" />
                          ...
                          <>
                            {rec.action === 'DELETE' ? '🗑️ Delete' :
                             rec.action === 'ARCHIVE' ? '📦 Archive' :
                             rec.action === 'COMPRESS' ? '🗜️ Compress' : '🛡️ Keep'}
                          </>
                        )}
                      </Button>
                    </div>
                    ...
                    >
                      {showAllRecommendations ? (
                        ...
                        <>
                          ...
                          Show {smartCleanupAnalysis.recommendations.length - 3} More Recommendations
                        </>
                      )}
                    </button>
                  </div>
        ...
        <div className="p-6 border-b border-gray-100">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={() => setIsProtectedFilesCollapsed(!isProtectedFilesCollapsed)}
          ...
          <div className="p-6">
            {isLoadingProtectedFiles ? (
              <div className="flex items-center justify-center py-8">
                ...
                <span className="text-gray-600">Loading protected files...</span>
              </div>
            ) : protectedFiles.length === 0 ? (
              <div className="text-center py-8">
                <Shield className="w-12 h-12 text-gray-300 mx-auto mb-3" />
                <p className="text-gray-600 mb-2">No files are currently protected</p>
                <p className="text-sm text-gray-500">
                  Use the Smart Cleanup Engine to protect important files from deletion
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                {protectedFiles.map((rule) => (
                  <div key={rule.id} className="flex items-center justify-between p-4 bg-emerald-50 border border-emerald-200 rounded-lg">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-sm font-medium text-gray-900 truncate">
                            {rule.filePath?.split('/').pop() || 'Protected File'}
                          </span>
                          <Badge variant="outline" className="text-xs border-emerald-300 text-emerald-700 bg-emerald-100">
                            {rule.protectionType}
                          </Badge>
                        </div>
                        <p className="text-xs text-gray-600 truncate">{rule.filePath}</p>
                        {rule.reason && (
                          <p className="text-xs text-emerald-700 mt-1">{rule.reason}</p>
                        )}
      ...
      <div className="bg-white border border-gray-200 rounded-xl">
        <div className="p-6 border-b border-gray-100">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => setIsTraditionalCleanupCollapsed(!isTraditionalCleanupCollapsed)}
              className="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors"
